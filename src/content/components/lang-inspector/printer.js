...
module.exports = {
  printReport(report) {
    ...
    const errorList = (warnList || [])
      ...
      .map(it => ({
        ...
        Pos: chalk.yellow(it.pos),
        message: chalk.red(it.msg)
      }));
    const warnsList = (warnList || [])
      .filter(it => it.level === "warn")
      .map(it => ({
        Type: chalk.green("Warn"),
        Pos: chalk.yellow(it.pos),
        message: chalk.cyan(it.msg)
      }));
  ...
  printSheetList(sheets, choose) {
    ...
    console.log(
      chalk.green("【多语言】当前链接检测到[") +
        chalk.yellow(sheets.length) +
        chalk.green("]张表：\n")
    );
